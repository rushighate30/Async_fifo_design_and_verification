...
(wr_clk, wr_rst, wr_en, rd_gry_sync, wr_addrs, fifo_full, wr_ptr_bin_gry);    // rollover bit addes in this condition
	...
	input [ADD_WIDTH:0] rd_gry_sync; // this is rd_2ff synch pointer 
  ...
  //  this is combinational next ptr taken to remove the lags which are created by the combination circuits
	wire [ADD_WIDTH:0] wr_ptr_bin_next = wr_ptr_bin + 1'b1;
   assign wr_addrs = wr_ptr_bin[ADD_WIDTH-1:0];   // 3 bit adress assign


	// compare the next gray pointer with the synchronized with read gray pointers
